...
export const createReportsSchema = z.object({
    ...
    patient_height: z.string().trim().nonempty("Patient Height is required.").refine((value) => {
        ...
        message: "Please enter a valid height or type 'N/A' if not applicable."
    }),
    ...
    form_2_details: z.object({
        ...
        // if (data.error_type_id === 13 && (!data.other_error_type || data.other_error_type.trim() === "")) {
        //     console.log("error: other error type is required", ctx);
        //     ctx.addIssue({
            ...
            if (data.medicines.length === 0) {
                ...
                data.medicines.forEach((med, index) => {
                    if (med.medicine_generic_id == null) {
                        ctx.addIssue({
                            path: ["medicines", index, "medicine_generic_id"],
    ...
    selected_error_type: z.string(),

    other_error_type: z.string().optional(),

    incident_description: z.string().nonempty("Description of the medication error incident is required."),
    workplace_environment: z.string().nonempty("Description of the workplace environment is required."),
    patient_condition: z.string().nonempty("Description of the patient condition is required."),
    immediate_actions: z.string().nonempty("Description of the immediate action/s done is required."),
    corrective_actions: z.string().nonempty("Description of the corrective action/s done is required."),
    preventive_actions: z.string().nonempty("Description of the preventive action/s done is required."),
    ...
    console.log("data.error_type_id?????", data);
    if (data.error_type_id === 13 && (!data.other_error_type || data.other_error_type.trim() === "")) {
        ctx.addIssue({
            path: ["other_error_type"],
            message: "Specify other error type is required.",
